...
% Testing Middle Cancelling Field
 ...
 s(113:123, 7:17) = 1; %bottom left
 s(113:123, 50:60) = 1; %bottom right
...
imp_res = feed_signal_into_IRS(x_obs,y_obs,z_obs,x_irs,y_irs,z_irs,k,delta);
%find_impulse_response(x_obs,y_obs,z_obs,x_irs,y_irs,z_irs,k, delta); %Feeds delta into our system giving us an impulse response
...
%  subplot(1,1,1)
%  imagesc(abs(sanity))
%  title('Final Signal s`(x,y) - Magnitude Only')
...
% subplot(1,1,1)
%  imagesc(abs(sanity))
%  title('Final Signal s`_p_h_a_s_e(x,y) - Phase and Magnitude')


for i=1:2 %iterations of phase

    new_s = get_phase(sanity, s, obs_Nx, obs_Ny);
    S = dtft_obs(new_s,x_obs,y_obs,kx,ky,obs_Nx,obs_Ny);
    S_OVER_H = S./H;
    r = inverse_dtft_rearr(S_OVER_H, irs_Nx, irs_Ny, x_irs, y_irs, kx, ky);
    sanity = feed_signal_into_IRS(x_obs,y_obs,z_obs,x_irs,y_irs,z_irs,k,r);
  ...
  subplot(1,1,1)
  imagesc(abs(sanity))
  title('s`25(x,y) - 25 Phase Iterations')
